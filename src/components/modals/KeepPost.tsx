...
interface KeepPostModalProps {
  isOpen: boolean;
  onClose: () => void;
  onConfirm: () => void;
}

export default function KeepPostModal({ isOpen, onClose, onConfirm }: KeepPostModalProps) {
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="w-[95%] max-w-md bg-white rounded-xl shadow-lg p-8 text-center border-none">
        <h2 className="text-xl font-bold text-gray-800 mb-2">Retain Post</h2>
        <p className="text-sm text-gray-600 mb-8">
          Keeping this post/comment will make it visible to users again. You can still remove it later if future reports provide stronger evidence.
        </p>

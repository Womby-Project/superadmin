...
interface ReasonForRemovalModalProps {
  isOpen: boolean;
  onClose: () => void;
  onConfirm: (reasons: string[]) => void;
}

export default function ReasonForRemovalModal({ isOpen, onClose, onConfirm }: ReasonForRemovalModalProps) {
  ...
  const handleConfirm = () => {
    if (selectedReasons.length > 0) {
      ...
      // Optional: show an alert or prevent closing if no reason is selected
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="w-[95%] max-w-lg bg-white rounded-xl shadow-lg p-8 border-none space-y-6">
        <div>
            <h2 className="text-xl font-bold text-gray-800">Reason for Removal</h2>
            <p className="text-sm text-gray-600 mt-1">
              Please provide a reason for removing this post. This will be shared with the user.
            </p>
        </div>
